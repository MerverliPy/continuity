# Canonical project state

Every material claim must occupy a row with its Evidence state, inspected Source reference, and stable Record ID. Narrative outside the table cannot elevate a claim to `Verified`.

| Material claim | Value | Evidence state | Source reference | Record ID |
| --- | --- | --- | --- | --- |
{{ canonical_state }}

The package lifecycle status in `MANIFEST.json` controls whether this recorded state is a Candidate or Canonical authority root.
